...
def insertuser(request):
    ...
    print(pname)
    # conn = Connect("127.0.0.1", "root", "", "goodreads")
    # s = "insert into admin1 values('" + request.POST["Email"] + "','" + request.POST["Password"] + "','" + request.PO[
    # "CO"] + "',0)"
    global conn
    s = f"insert into usersignup values('" + email + "','" + password + "','" + confirmpassword + "','" + name + "','" + address + "','" + state + "','" + mobile + "','" + pname + "')"
    fs = FileSystemStorage()
    fs.save(pname, file)
    cr = conn.cursor()
    cr.execute(s)
...
def userlogin(request):
    ...
    if result:
        ...
        return request(request, "userlogin.html")
...
def showgenre(request):
    ...
    for row in result:
        if not bookstitle:
            ...
            dist['catname'] = catname
            bookdata.append(dist)
    return render(request, 'bookgenre.html', {'bookdata': bookdata})
...
def processtopay(request):
    ...
    if 'Email' not in request.session:
        ...
        for row in request.session['addtocart']:
            total += float(row['price'])
        email = request.session["Email"]
        query = f"SELECT `email`, `password`, `confirmpassword`, `name`, `address`, `state`, `mobile`, `photo` FROM `usersignup` WHERE email='{email}'"
        result = Fetchone(query)
        print(result)
        userdata = {'email': email, 'mobile': result[6], 'name': result[3], 'address': result[4], 'total': total}
